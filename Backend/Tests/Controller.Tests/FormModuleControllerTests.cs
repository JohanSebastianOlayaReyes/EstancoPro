...
namespace Controller.Tests
{
    public class FormModuleControllerTests
    {
        ...
        {
            var mock = new Mock<IFormModuleBusiness>();
            var list = new List<FormModuleDto> { new FormModuleDto { FormId = 1, ModuleId = 2 } };
            mock.Setup(m => m.GetAllAsync()).ReturnsAsync(list);

            var controller = new FormModuleController(mock.Object);
